...
// set type inital value (expression tree)
...
A_TYPE *setTypeStructOrEnumIdentifier(T_KIND k, char *s, ID_KIND kk)
{
    A_TYPE *t;
    A_ID *id, *a;
    // check redeclaration or forward declartion
...
void initialize()
{
    ...
    setDeclaratorTypeAndKind(
        makeIdentifier("printf"), setTypeField(setTypeElementType(makeType(T_FUNC), void_type), linkDeclaratorList(setDeclaratorTypeAndKind(makeDummyIdentifier(), string_type, ID_PARM), setDeclaratorKind(makeDummyIdentifier(), ID_PARM))), ID_FUNC);
...
void syntax_error(int i, char *s)
{
    ...
    {
    case 11:
        printf("illegal referencing struct of union identifier %s", s);
        break;
    case 12:
